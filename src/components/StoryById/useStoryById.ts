...
import { getStoryById, getQuestions, submitFeedback, getWordDetails, saveWords, clearStory, type Question, type WordDetailsResponse } from "../../store/storiesSlice"
import { useAppDispatch, useAppSelector } from "../../store/store"
import type { SavedWord } from "../../types"
import { useSpeechSynthesis } from "../../hooks/useSpeechSynthesis"
import { useTranslation } from "../../i18n/useTranslation"
import type { PopoverPosition } from "../StoryReader/types"
...
export function useStoryById() {
  ...
  const isLoadingStory = useAppSelector((state) => state.stories.isLoadingStory)
  ...
  const storyNotFound = useAppSelector((state) => state.stories.storyNotFound)
  const storyAlreadyRead = useAppSelector((state) => state.stories.storyAlreadyRead)
  ...
  // Fetch story by ID on component mount
  useEffect(() => {
    if (hasFetchedStory.current || !id) return

    const fetchStory = async () => {
      hasFetchedStory.current = true
      try {
        const result = await dispatch(getStoryById(id)).unwrap()

        dispatch(setStoryId(result.story.id))
        dispatch(setStoryText(result.story.story))
        dispatch(setTranslations(result.story.translations))
  ...
  const handleComplete = useCallback(async () => {
    if (!storyId || storyAlreadyRead) return
  ...
  }, [dispatch, storyId, savedWords, questions, attemptCounts, likeStatus, storyAlreadyRead, navigate])
